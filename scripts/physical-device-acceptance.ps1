...
try {
    ...
    $passed = $instrument.ExitCode -eq 0 -and
        $instrumentText -match "OK \(5 tests\)" -and
    ...
    if (-not $passed) {
        $failure = if ($instrument.TimedOut) { "Instrumentation exceeded 600 seconds." } else { "Instrumentation or its required evidence did not report a complete 5-test pass." }
    }
} catch {
...
Write-Host "PASS: all 5 physical CameraX tests passed."
