...
The repeatable 12-call semantic harness is `scripts/qwen-intent-smoke.py`. It reads only `EVALUATION_MODEL_NAME` and `EVALUATION_MODEL_KEY` from `.env`, writes redacted JSONL under ignored `outputs/qa/`, and never writes an image or key copy. The prior content-hash-matched run passed 12/12 strict complaint-intent cases; a final clean-provenance rerun is pending. Qwen may return only an allowlisted semantic label or clarification reason; Android reparses it and plans all values and camera actions locally.
