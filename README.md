...
Voice input is never always-on. Tap **Mic** to listen for one utterance; Android may end it automatically, or tap **Done** to request the final transcript. Sending the app to the background cancels listening. A stalled recognition request times out after 20 seconds and returns to the typed-comment fallback. The physical acceptance runner preserves the phone's existing English locale rather than changing it.
...
- JVM: 159 tests pass with 0 failures, 0 errors, and 0 skips, including compound classification/planning, fresh-telemetry replanning, all-or-none capability gates, cancellation, rollback, session/lens invalidation, one Reset, queued voice finalization/timeout, and hosted-scalar fail-closed behavior.
...
- Typed and injected voice-result paths reach the same safe intent chain. The physical reports from `20260809-162043`, `20260809-163230`, `20260809-163717`, and the rebuilt real-UI run `20260809-170120` prove that Android's recognizer was ready before Edge playback and that Edge completed playback before the runner pressed **Done** for the final transcript. Local TTS, Edge neural TTS, and a commit-pinned human recording all reached the phone microphone and SODA, but this OnePlus provider returned an empty final result followed by error 7 for each. The acoustic speech-to-text gate therefore remains red on this device/provider combination.
