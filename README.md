...
The current repeatable 14-call semantic harness is `scripts/qwen-intent-smoke.py`. It reads only `EVALUATION_MODEL_NAME` and `EVALUATION_MODEL_KEY` from `.env`, writes redacted JSONL under ignored `outputs/qa/`, and never writes an image or key copy. In the clean schema-v2 run at commit `13aff93`, 9/14 cases returned the expected result within the 15-second complaint deadline; three additional responses were exact but late, one call disconnected, and one safely clarified instead of returning the expected darker intent. This does not qualify Qwen as a reliable dependency. Qwen may return only one allowlisted semantic label, a two- or three-label direct-setting list, or an allowlisted clarification reason. Android strictly reparses that result and plans all values and camera actions locally, while provider failures remain optional and fail-closed.
